...
def load_passages(ctx_file: str, remove_lang) -> Dict[object, Tuple[str, str]]:
    docs = {}
    for lang in ['en', 'ar', 'fa', 'fr', 'id', 'pt', 'ru']:
        if remove_lang is not None and lang in remove_lang: continue
    # for lang in ['en']:
        for idx, f in enumerate(sorted(glob(os.path.join(ctx_file,f'{lang}/*.txt')))):
            
            # docs[f'{lang}-{idx}'] = open(f,'r').read()#.split('<sep>')[1]
            # docs[f'{lang}.mbert-{idx}'] = open(f,'r').read().split('<sep>')[1]
            docs[f'{lang}.xict-{idx}'] = open(f,'r').read()#.split('<sep>')[1]
            # assert '<sep>' not in docs[f'{lang}.xict-{idx}']
            # assert '<sep>' in open(f,'r').read() 
    #  = {idx:open(f,'r').read() for idx, f in enumerate(sorted(glob(os.path.join(ctx_file,'*.txt'))))}
...
def main(args):
    ...
    for lang in ['en', 'ar', 'fa', 'fr', 'id', 'pt', 'ru']:
    # for lang in ['en']:
        # path = os.path.join(args.encoded_dir, f'emb_{lang}.pkl')
        path = os.path.join(args.encoded_dir, f'emb_{lang}.xict.pkl')
        # path = os.path.join(args.encoded_dir, f'emb_{lang}.mbert.pkl')
        
        if args.remove_lang is None or lang not in args.remove_lang :
            ...
            print("lang {} are removed from retrieval target".format(lang))
    ...
    claims_df = read_data(args.claim_file)

    questions = claims_df.claim.tolist()
    question_answers = [None] * len(questions) # we don't have gold retrieval label
    question_languages = claims_df.language.tolist()
    q_ids = np.arange(len(questions)).tolist()
    ...
    # ctx_file: dir to the passages
    all_passages = load_passages(args.ctx_file, args.remove_lang)
    ...
    questions_doc_hits = [0] * len(questions)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--claim_file', required=True, type=str, default=None,
                        help="A tsv file containing claim.")
